#!/usr/bin/env bash
# PR #3 のユーザーテスト手順を自動実行する（APIレベルpytest + Playwrightブラウザ確認）。
#
# 前提: 開発サーバーが起動していること
...
echo "== PR3: APIレベルテスト (pytest) =="
"$PY" -m pytest "$REPO_ROOT/test/pr3/test_pr3_api.py" -v
API_STATUS=$?
echo "(pytest exit status: $API_STATUS)"

echo
echo "== PR3: ブラウザテスト (Playwright) =="
if PW_NODE_MODULES="$(find_playwright_node_modules)"; then
  "$PY" "$REPO_ROOT/test/support/gen_fixtures.py" "$FIXTURES_DIR"
  NODE_PATH="$PW_NODE_MODULES" node "$REPO_ROOT/test/pr3/browser_pr3.cjs" "$FIXTURES_DIR"
  BROWSER_STATUS=$?
else
